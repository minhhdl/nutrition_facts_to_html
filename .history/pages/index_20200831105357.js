...
const analyzeText = (fullText, ocrResult) =>
  new Promise((resolve, reject) => {
    ...
    for (let rowIndex in fullText) {
      ...
      for (let index in ocrResult) {
        if (row.includes(ocrResult[index].description)) {
          console.log(row, ocrResult[index].description);
          byRow[rowIndex] = byRow[rowIndex]
            ? [...byRow[rowIndex], ocrResult[index]]
            : [ocrResult[index]];
          matchedIndexes.push(index);
        }
        if (matchedIndexes.length === row.length) break;
      }
      for (let i of matchedIndexes) {
        ocrResult.splice(i, 1);
      }
    }
    console.log(byRow);
  });
