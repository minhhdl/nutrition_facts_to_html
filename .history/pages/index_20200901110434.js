...
export default function Home() {
  ...
  const handleUpload = async () => {
    try {
      ...
      const fullTextResult = result.data.textAnnotations[0].description.split(
        '\n',
      );
      const byRow = await analyzeText(
        fullTextResult,
        result.data.textAnnotations.slice(1),
      );
      setTextByLine(byRow);
      setFullText(result.data.textAnnotations[0].description.split('\n'));
      localStorage.setItem('byRow', byRow);
      localStorage.setItem('fullText', fullTextResult);
      localStorage.setItem('textAnnotations', result.data.textAnnotations);
    } catch (e) {}
  };
