...
export default function Home() {
  ...
  const convertHtml = () => {
    const heightRatio =
      imageRef?.current.clientHeight < container.height
        ? imageRef?.current.clientHeight / container.height
        : container.height / imageRef?.current.clientHeight;
    ...
    const html = ReactDOMServer.renderToStaticMarkup(
      ...
      >
        {Object.keys(textByLine).map((key) => {
          ...
          const [tlPoint, trPoint, brPoint, blPoint] = vertices;
          const height =
            (((blPoint.y - tlPoint.y) * 100) / container.height) * heightRatio;
          const top = (tlPoint.y / container.height) * 100 * heightRatio;
          const left = (tlPoint.x / container.width) * 100 * heightRatio;
